"""GSC MCP Server — Google Search Console via MCP.

Exposes Google Search Console tools via MCP stdio transport. Supports both
OAuth (interactive) and service account (headless) authentication.

Usage:
    GSC_CREDENTIALS_PATH=/path/to/sa.json python gsc_server.py
    python gsc_server.py   # uses OAuth flow or local service_account_credentials.json
"""
...
# Heavy Google client libraries (googleapiclient.discovery, google_auth_oauthlib,
# google.oauth2) are imported LAZILY via _ensure_google_imports(). At module top
# level they delay the MCP stdio `initialize` handshake under cold-start
# contention. Deferring them lets FastMCP answer the handshake immediately; the
# import cost moves to the first auth call. The names below are populated as
# module globals on first use.
...
def _ensure_google_imports() -> None:
    """Import the heavy Google client libraries on first use (idempotent).

    Called by the auth gateways before any Google API work. `HttpError` is only
    referenced in `except` clauses, which are reached only after an auth call has
    run this loader — so the name is always populated by then.
    """
    global Request, service_account, Credentials, InstalledAppFlow, build, HttpError
...
async def get_search_analytics(
    site_url: str, days: int = 28, dimensions: str = "query", row_limit: int = 20
) -> str:
    """
    ...
    try:
        ...
        for row in response.get("rows", []):
            ...
            for dim_value in row.get("keys", []):
                data.append(
                    dim_value[:100]
                )  # Increased truncation limit to 100 characters
...
async def get_site_details(site_url: str) -> str:
    ...
    try:
        ...
        if "siteVerificationInfo" in site_info:
            ...
            result_lines.append(
                f"Verification state: {verify_info.get('verificationState', 'Unknown')}"
            )
            ...
            if "verificationMethod" in verify_info:
                result_lines.append(
                    f"Verification method: {verify_info['verificationMethod']}"
                )
        ...
        if "ownershipInfo" in site_info:
            ...
            if "verificationMethod" in owner_info:
                result_lines.append(
                    f"Ownership verification: {owner_info['verificationMethod']}"
                )
...
async def get_sitemaps(site_url: str) -> str:
    ...
    try:
        ...
        for sitemap in sitemaps.get("sitemap", []):
            ...
            result_lines.append(
                f"{path} | {last_downloaded} | {status} | {indexed_urls} | {errors}"
            )
...
async def inspect_url_enhanced(site_url: str, page_url: str) -> str:
    ...
    try:
        ...
        if "inspectionResultLink" in inspection:
            result_lines.append(
                f"Search Console Link: {inspection['inspectionResultLink']}"
            )
            result_lines.append("-" * 80)
        ...
        if "lastCrawlTime" in index_status:
            try:
                ...
                result_lines.append(
                    f"Last Crawled: {crawl_time.strftime('%Y-%m-%d %H:%M')}"
                )
            except Exception:
        ...
        if "userCanonical" in index_status and index_status.get(
            "userCanonical"
        ) != index_status.get("googleCanonical"):
        ...
        if "referringUrls" in index_status and index_status["referringUrls"]:
            ...
            if len(index_status["referringUrls"]) > 5:
                result_lines.append(
                    f"... and {len(index_status['referringUrls']) - 5} more"
                )
            ...
            if "detectedItems" in rich and rich["detectedItems"]:
                ...
                for item in rich["detectedItems"]:
                    ...
                    if "items" in item and item["items"]:
                        ...
                        if len(item["items"]) > 3:
                            result_lines.append(
                                f"  • ... and {len(item['items']) - 3} more items"
                            )
    ...
    try:
        ...
        for page_url in url_list:
            ...
            try:
                ...
                response = (
                    service.urlInspection().index().inspect(body=request).execute()
                )
                ...
                if "richResultsResult" in inspection:
                    rich = inspection["richResultsResult"]
                    if (
                        rich.get("verdict") == "PASS"
                        and "detectedItems" in rich
                        and rich["detectedItems"]
                    ):
                        rich_types = [
    ...
    try:
        ...
        for page_url in url_list:
            ...
            try:
                ...
                if (
                    verdict != "PASS"
                    or "not indexed" in coverage.lower()
                    or "excluded" in coverage.lower()
                ):
...
async def get_performance_overview(site_url: str, days: int = 28) -> str:
    ...
    try:
        ...
        total_response = (
            ...
            .query(siteUrl=site_url, body=total_request)
            .execute()
        )
        ...
        date_response = (
            service.searchanalytics()
            .query(siteUrl=site_url, body=date_request)
            .execute()
        )
        ...
        if date_response.get("rows"):
            ...
            for row in sorted_rows:
                ...
                result_lines.append(
                    f"{date_formatted} | {clicks:.0f} | {impressions:.0f} | {ctr:.2f}% | {position:.1f}"
                )
...
async def get_advanced_search_analytics(
    ...
    try:
        ...
        if not start_date:
            start_date = (datetime.now().date() - timedelta(days=28)).strftime(
                "%Y-%m-%d"
            )
        ...
        if not response.get("rows"):
            ...
            if active_filters:
                ...
                for f in active_filters:
                    no_data_msg += (
                        f"    {f['dimension']} {f['operator']} '{f['expression']}'\n"
                    )
            else:
        ...
        result_lines.append(
            f"Showing rows {start_row + 1} to {start_row + len(response.get('rows', []))} (sorted by {sort_by} {sort_direction})"
        )
        result_lines.append("\n" + "-" * 80 + "\n")
...
async def compare_search_periods(
    ...
    try:
        ...
        period1_response = (
            service.searchanalytics()
            .query(siteUrl=site_url, body=period1_request)
            .execute()
        )
        period2_response = (
            service.searchanalytics()
            .query(siteUrl=site_url, body=period2_request)
            .execute()
        )
        ...
        for key in all_keys:
            ...
            pos_diff = p1_row.get("position", 0) - p2_row.get(
                "position", 0
            )  # Note: lower position is better
        ...
        result_lines.append(
            f"{dim_header} | P1 Clicks | P2 Clicks | Change | % | P1 Pos | P2 Pos | Pos Δ"
        )
        ...
        for item in comparison_data[:limit]:
            ...
            click_pct = (
                item["click_pct"] if item["click_pct"] != float("inf") else "N/A"
            )
...
async def get_search_by_page_query(
    site_url: str, page_url: str, days: int = 28, row_limit: int = 20
) -> str:
    """
    ...
    try:
        ...
        for row in response.get("rows", []):
            ...
            result_lines.append(
                f"{query[:100]} | {clicks} | {impressions} | {ctr:.2f}% | {position:.1f}"
            )

        # Add total metrics
        total_clicks = sum(row.get("clicks", 0) for row in response.get("rows", []))
        total_impressions = sum(
            row.get("impressions", 0) for row in response.get("rows", [])
        )
        avg_ctr = (
        ...
        result_lines.append("-" * 80)
        result_lines.append(
            f"TOTAL | {total_clicks} | {total_impressions} | {avg_ctr:.2f}% | -"
        )
...
async def list_sitemaps_enhanced(site_url: str, sitemap_index: str = None) -> str:
    ...
    try:
        ...
        for sitemap in sitemaps.get("sitemap", []):
            ...
            # Determine type
            sitemap_type = (
                "Index" if sitemap.get("isSitemapsIndex", False) else "Sitemap"
            )
            ...
            result_lines.append(
                f"{path} | {last_submitted} | {last_downloaded} | {sitemap_type} | {url_count} | {errors} | {warnings}"
            )
        ...
        pending_count = sum(
            1
            for sitemap in sitemaps.get("sitemap", [])
            if sitemap.get("isPending", False)
        )
        if pending_count > 0:
            result_lines.append(
...
async def get_sitemap_details(site_url: str, sitemap_url: str) -> str:
    ...
    try:
        ...
        result_lines.append(
            f"Status: {'Pending processing' if is_pending else 'Processed'}"
        )
        ...
        if "lastSubmitted" in details:
            try:
                dt = datetime.fromisoformat(
                    details["lastSubmitted"].replace("Z", "+00:00")
                )
        ...
        if "lastDownloaded" in details:
            try:
                dt = datetime.fromisoformat(
                    details["lastDownloaded"].replace("Z", "+00:00")
                )
        ...
        if "contents" in details and details["contents"]:
            ...
            for content in details["contents"]:
                ...
                result_lines.append(
                    f"- {content_type}: {submitted} submitted, {indexed} indexed"
                )
        ...
        try:
            ...
            if "lastSubmitted" in details:
                try:
                    ...
                    result_lines.append(
                        f"Submission time: {dt.strftime('%Y-%m-%d %H:%M')}"
                    )
                except Exception:
            ...
            result_lines.append(
                f"Status: {'Pending processing' if is_pending else 'Processing started'}"
            )
...
async def manage_sitemaps(
    site_url: str, action: str, sitemap_url: str = None, sitemap_index: str = None
) -> str:
    """
